...
function createTables() {
  const ddl = [
    ...
    `create table if not exists contracts (id text primary key, code text not null unique, name text not null, customer_id text not null references customers(id), opportunity_id text references opportunities(id), party_a text not null, party_b text not null, total_amount integer not null default 0, payment_method text, start_date text, end_date text, status text not null default 'draft', reject_reason text, approved_by text references users(id), approved_at text, owner_user_id text references users(id), created_by text not null references users(id), remark text, content text, supplier_id text, created_at text not null default (datetime('now')), updated_at text not null default (datetime('now')), deleted_at text)`,

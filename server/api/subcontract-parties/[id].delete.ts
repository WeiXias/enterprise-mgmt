...
export default defineEventHandler(async (event) => {
  ...
  await db.update(subcontractParties).set({ deletedAt: now }).where(eq(subcontractParties.id, id))

  await logOperation(event, { action: 'DELETE', targetId: id, detail: '删除了分包方' })

...
export default defineEventHandler(async (event) => {
  ...
  await db.update(contracts).set({ ownerUserId: parsed.data.toUserId, updatedAt: now }).where(eq(contracts.id, id))

  await logOperation(event, { action: 'UPDATE', module: 'contract', targetId: id, detail: { transferTo: parsed.data.toUserId, reason: parsed.data.reason } })

  return { code: 0, data: null, message: '合同已转交' }
})

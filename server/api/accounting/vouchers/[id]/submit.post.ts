...
export default defineEventHandler(async (event) => {
  ...
  if (existing[0].status !== 'draft') throw createError({ statusCode: 403, statusMessage: '只能提交草稿状态的凭证' })
  ...
  await logOperation(event, { action: 'SUBMIT', module: 'accounting', targetId: id, detail: '提交审核凭证' })
  return { code: 0, message: '已提交审核' }
})

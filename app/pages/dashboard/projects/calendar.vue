...
function statusColor(s: string) {
  return { todo: 'ring-gray-300', in_progress: 'ring-blue-400', completed: 'ring-teal-400' }[s] || 'ring-gray-300'
}
...
<template>
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-lg font-medium text-gray-800">项目日历</h1>
        <p class="text-sm text-gray-400 mt-0.5">按日期查看所有项目任务</p>
      </div>
      <div class="flex items-center gap-2">
        <select v-model="projectFilter" class="px-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400 bg-white">
          <option value="">全部项目</option>
          <option v-for="p in projectOptions" :key="p.id" :value="p.id">{{ p.name }}</option>
        </select>
        <div class="flex items-center bg-gray-100 rounded-lg p-0.5">
          <button :class="['px-3 py-1 text-xs rounded-md transition-colors', viewMode === 'month' ? 'bg-white shadow-sm text-gray-700' : 'text-gray-400']" @click="viewMode = 'month'">月</button>
          <button :class="['px-3 py-1 text-xs rounded-md transition-colors', viewMode === 'week' ? 'bg-white shadow-sm text-gray-700' : 'text-gray-400']" @click="viewMode = 'week'">周</button>
        </div>
      </div>
    </div>
    ...
    <div class="flex gap-6">
      ...
      <div class="flex-1">
        ...
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm font-medium text-gray-700 w-28 text-center">{{ viewMode === 'month' ? dayjs(currentMonth).format('YYYY年 M月') : dayjs(currentWeekStart).format('M月D日') + ' - ' + dayjs(currentWeekStart).add(6, 'day').format('M月D日') }}</span>
        ...
        <div v-if="viewMode === 'month'">
          <div class="grid grid-cols-7 gap-px bg-gray-100 rounded-lg overflow-hidden">
            <div v-for="wd in WEEKDAYS" :key="wd" class="text-center py-2 text-xs text-gray-400 font-medium bg-white">{{ wd }}</div>
            <div
              v-for="d in monthDays" :key="dateKey(d)"
              :class="['min-h-[80px] p-1 bg-white cursor-pointer hover:bg-gray-50 transition-colors border-t border-gray-100', dateKey(d) === selectedDate ? 'ring-2 ring-brand-400 ring-inset' : '', d.month() !== dayjs(currentMonth).month() ? 'opacity-30' : '']"
              @click="selectDate(d)"
            >
              <div class="flex items-center justify-between mb-0.5">
                <span :class="['text-xs w-5 h-5 flex items-center justify-center rounded-full', dateKey(d) === dayjs().format('YYYY-MM-DD') ? 'bg-brand-400 text-white' : 'text-gray-500']">{{ d.date() }}</span>
        ...
        <div v-if="viewMode === 'week'">
          <div class="grid grid-cols-7 gap-px bg-gray-100 rounded-lg overflow-hidden">
            <div v-for="d in weekDays" :key="dateKey(d)" :class="['min-h-[200px] p-2 bg-white cursor-pointer hover:bg-gray-50 transition-colors', dateKey(d) === selectedDate ? 'ring-2 ring-brand-400 ring-inset' : '']" @click="selectDate(d)">
              <div class="text-center mb-2">
                <p class="text-[10px] text-gray-400">{{ WEEKDAYS[d.day() === 0 ? 6 : d.day() - 1] }}</p>
                <p :class="['text-sm w-6 h-6 mx-auto flex items-center justify-center rounded-full', dateKey(d) === dayjs().format('YYYY-MM-DD') ? 'bg-brand-400 text-white' : 'text-gray-700']">{{ d.date() }}</p>
              </div>
              <div class="space-y-1">
                <div v-for="(t, i) in (calendarTasks[dateKey(d)] || []).slice(0, 6)" :key="i" :class="['text-[10px] px-1.5 py-0.5 rounded truncate', priorityColor(t.priority).replace('bg-', 'bg-opacity-20 bg-'), 'text-gray-600 border-l-2', statusColor(t.status).replace('ring-', 'border-')]">{{ t.title }}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="selectedDate" class="w-64 flex-shrink-0">
        <div class="warm-card sticky top-6">
          <h3 class="text-sm font-medium text-gray-700 mb-2">{{ selectedDate }}</h3>
          <div v-if="selectedDateTasks.length === 0" class="text-xs text-gray-400 py-3 text-center">这天没有任务</div>
          <div v-else class="space-y-2">
            <div v-for="t in selectedDateTasks" :key="t.id" class="p-2 rounded-lg bg-gray-50 text-xs">
              <div class="flex items-center gap-1 mb-0.5">
                <div :class="['w-1.5 h-1.5 rounded-full', priorityColor(t.priority)]" />
                <span class="text-gray-700 truncate">{{ t.title }}</span>
              </div>

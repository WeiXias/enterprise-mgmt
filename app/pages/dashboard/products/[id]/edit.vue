...
async function fetchData() {
  try {
    ...
    if (prodRes?.code === 0) {
      ...
      // 已有规格
      if (p.specs?.length) {
        selectedSpecTemplate.value = p.specs[0].specTemplate || ''
        await loadSpecTemplate(selectedSpecTemplate.value)
        for (const s of p.specs) {
          specValues.value[s.specKey] = s.specValue || ''
        }
      }
    }
...
<template>
  <div class="max-w-lg mx-auto">
    <div class="mb-6 flex items-center gap-3">
      ...
      <h1 class="text-lg font-medium text-content-primary">编辑产品</h1>
    </div>
    <div v-if="loading" class="text-center py-12 text-content-muted">加载中...</div>
    <div v-else class="em-card space-y-6">
      <!-- 基本信息 -->
      <div>
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-3 mb-3">
          <div v-for="(img, idx) in images" :key="idx" class="relative w-20 h-20 rounded-lg border border-line overflow-hidden group">
            ...
            <img v-else-if="img.filePath" :src="img.filePath.startsWith('/uploads') ? img.filePath : `/api/attachments/${img.fileName}`" class="w-full h-full object-cover" />

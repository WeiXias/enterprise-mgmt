...
async function handleSubmit() {
  ...
  try {
    ...
    if (res?.code === 0) { toast.add({ title: '产品已添加', color: 'success' }); router.push('/dashboard/products') }
...
<template>
  <div class="max-w-lg mx-auto">
    <div class="mb-6 flex items-center gap-3">
      ...
      <h1 class="text-lg font-medium text-stone-800">新建产品</h1>
    </div>
    <div class="warm-card">
      <form class="space-y-4" @submit.prevent="handleSubmit">
        <div><label class="block text-sm text-stone-600 mb-1">名称 <span class="text-red-400">*</span></label><input v-model="form.name" type="text" placeholder="产品名称" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" /></div>
        <div><label class="block text-sm text-stone-600 mb-1">编码 <span class="text-red-400">*</span></label><input v-model="form.code" type="text" placeholder="SKU-001" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" /></div>
        ...
        <div><label class="block text-sm text-stone-600 mb-1">描述</label><textarea v-model="form.description" rows="2" placeholder="产品描述..." class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 resize-none" /></div>
      </form>

  ...
  <div v-else>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="em-card">
        ...
        <div v-if="recentTransactions.length === 0" class="text-xs text-content-secondary py-4 text-center">暂无流水</div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
      ...
      <NuxtLink to="/dashboard/finance/transactions" class="em-card flex items-center gap-3 hover:shadow-sm transition-shadow cursor-pointer p-4">
        ...
        <div><p class="text-sm text-content-primary">登记收入</p><p class="text-xs text-content-secondary">记录合同回款</p></div>
      </NuxtLink>
      ...
      <NuxtLink to="/dashboard/finance/reports" class="em-card flex items-center gap-3 hover:shadow-sm transition-shadow cursor-pointer p-4">
    ...
    <FormModal v-if="showQuickModal" v-model:open="showQuickModal" title="记一笔" subtitle="快速记录一笔收入或支出" size="compact" :loading="quickLoading" @confirm="handleQuickCreate">
      ...
      <div class="mb-3">
        ...
        <EnumSelect v-model="quickForm.category" :options="getCategories()" placeholder="选择分类" />
      </div>
      <div class="mb-3">
        <label class="block text-sm text-content-secondary mb-1">金额 <span class="text-danger-600">*</span></label>
        <input v-model.number="quickForm.amount" type="number" step="0.01" placeholder="0.00" class="w-full input-base focus-ring" />
      </div>
      <div class="mb-3">
        <label class="block text-sm text-content-secondary mb-1">日期</label>
        <input v-model="quickForm.transactionDate" type="date" class="w-full input-base focus-ring" />
      </div>
      <div>

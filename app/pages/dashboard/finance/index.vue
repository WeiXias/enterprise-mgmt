...
definePageMeta({ layout: 'dashboard', title: '财务总览', middleware: ['auth'] })
...
<template>
  ...
  <div v-else>
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-lg font-medium text-stone-800">财务总览</h1>
        <p class="text-sm text-stone-400 mt-0.5">看看财务状况怎么样</p>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <div class="warm-card flex items-center gap-4">
        ...
        <div><p class="text-xl font-semibold text-teal-600">{{ formatMoney(stats.totalIncome) }}</p><p class="text-xs text-stone-400">总收入</p></div>
      </div>
      <div class="warm-card flex items-center gap-4">
        ...
        <div><p class="text-xl font-semibold text-red-500">{{ formatMoney(stats.totalExpense) }}</p><p class="text-xs text-stone-400">总支出</p></div>
      </div>
      <div class="warm-card flex items-center gap-4">
        ...
        <div><p class="text-xl font-semibold" :class="stats.netBalance >= 0 ? 'text-teal-600' : 'text-red-500'">{{ formatMoney(stats.netBalance) }}</p><p class="text-xs text-stone-400">净利润</p></div>
      </div>
      <div class="warm-card flex items-center gap-4">
        <div class="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center"><UIcon name="i-lucide-bell" class="w-5 h-5 text-blue-400" /></div>
        <div>
          <p class="text-xl font-semibold text-stone-700">{{ stats.overduePaymentCount + stats.pendingReimbursementCount }}</p>
          <p class="text-xs text-stone-400">待处理（{{ stats.overduePaymentCount }} 逾期 + {{ stats.pendingReimbursementCount }} 报销）</p>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <!-- 逾期回款提醒 -->
      <div class="warm-card">
        <h3 class="text-sm font-medium text-stone-700 mb-3">逾期未回款</h3>
        <div v-if="overduePayments.length === 0" class="text-xs text-stone-400 py-4 text-center">没有逾期回款，继续保持！</div>
        <div v-else class="space-y-2">
          <div v-for="p in overduePayments" :key="p.id" class="flex items-center justify-between p-2 rounded-lg bg-red-50">
            <div>
              <span class="text-sm text-stone-700">{{ p.contractName }} ({{ p.contractCode }})</span>
              <span class="text-xs text-stone-400 ml-2">{{ p.planDate }}</span>
            </div>
            <div class="flex items-center gap-2">
      ...
      <div class="warm-card">
        <div class="flex items-center justify-between mb-3">
          <h3 class="text-sm font-medium text-stone-700">最近流水</h3>
          <NuxtLink to="/dashboard/finance/transactions" class="text-xs text-amber-600 hover:underline">查看全部</NuxtLink>
        </div>
        <div v-if="recentTransactions.length === 0" class="text-xs text-stone-400 py-4 text-center">暂无流水</div>
        <div v-else class="space-y-1.5">
          <div v-for="t in recentTransactions" :key="t.id" class="flex items-center justify-between p-2 rounded-lg hover:bg-stone-50 text-xs">
            <div class="flex items-center gap-2">
              ...
              <span class="text-stone-700">{{ t.description || t.category }}</span>
              <span class="text-stone-400">{{ t.transactionDate }}</span>
            </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
      <NuxtLink to="/dashboard/finance/invoices" class="warm-card flex items-center gap-3 hover:shadow-sm transition-shadow cursor-pointer p-4">
        ...
        <div><p class="text-sm text-stone-700">发票管理</p><p class="text-xs text-stone-400">开票和追踪</p></div>
      </NuxtLink>
      <NuxtLink to="/dashboard/finance/transactions" class="warm-card flex items-center gap-3 hover:shadow-sm transition-shadow cursor-pointer p-4">
        ...
        <div><p class="text-sm text-stone-700">登记收入</p><p class="text-xs text-stone-400">记录合同回款</p></div>
      </NuxtLink>
      <NuxtLink to="/dashboard/finance/reimbursements" class="warm-card flex items-center gap-3 hover:shadow-sm transition-shadow cursor-pointer p-4">
        <div class="w-8 h-8 rounded-lg bg-amber-50 flex items-center justify-center"><UIcon name="i-lucide-receipt" class="w-4 h-4 text-amber-500" /></div>
        <div><p class="text-sm text-stone-700">提交报销</p><p class="text-xs text-stone-400">登记日常支出</p></div>
      </NuxtLink>
      <NuxtLink to="/dashboard/finance/reports" class="warm-card flex items-center gap-3 hover:shadow-sm transition-shadow cursor-pointer p-4">
        <div class="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center"><UIcon name="i-lucide-file-spreadsheet" class="w-4 h-4 text-blue-400" /></div>
        <div><p class="text-sm text-stone-700">月度报表</p><p class="text-xs text-stone-400">看看赚了多少</p></div>
      </NuxtLink>
      <div class="warm-card flex items-center gap-3 hover:shadow-sm transition-shadow cursor-pointer p-4" @click="showQuickModal = true">
        ...
        <div><p class="text-sm text-stone-700">登记支出</p><p class="text-xs text-stone-400">记录日常开销</p></div>
      </div>
    </div>

    <!-- 快捷登记弹窗 -->
    <UModal v-model:open="showQuickModal">
      <template #header>记一笔</template>
      <template #body>
        <form class="space-y-3" @submit.prevent="handleQuickCreate">
          ...
          <div><label class="block text-sm text-stone-600 mb-1">分类</label><select v-model="quickForm.category" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 bg-white"><option value="">选择分类</option><option v-for="c in getCategories()" :key="c" :value="c">{{ c }}</option></select></div>
          <div><label class="block text-sm text-stone-600 mb-1">金额 <span class="text-red-400">*</span></label><input v-model.number="quickForm.amount" type="number" step="0.01" placeholder="0.00" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" /></div>
          <div><label class="block text-sm text-stone-600 mb-1">日期</label><input v-model="quickForm.transactionDate" type="date" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" /></div>
          <div><label class="block text-sm text-stone-600 mb-1">说明</label><input v-model="quickForm.description" type="text" placeholder="简单描述一下..." class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" /></div>
        </form>
      </template>
      <template #footer>
        <div class="flex justify-end gap-2"><UButton variant="ghost" color="neutral" @click="showQuickModal = false">取消</UButton><UButton color="primary" :loading="quickLoading" @click="handleQuickCreate">保存</UButton></div>
      </template>
    </UModal>
  </div>
</template>

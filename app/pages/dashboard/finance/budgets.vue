...
definePageMeta({ layout: 'dashboard', title: '预算管理', middleware: ['auth'] })
...
<template>
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-lg font-medium text-stone-800">预算管理</h1>
        <p class="text-sm text-stone-400 mt-0.5">设定年度预算，追踪执行进度</p>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
      <div class="warm-card flex items-center gap-3 !py-3">
        ...
        <div><p class="text-lg font-semibold text-stone-700">{{ formatMoney(totalStats.budgetTotal) }}</p><p class="text-xs text-stone-400">预算总额</p></div>
      </div>
      <div class="warm-card flex items-center gap-3 !py-3">
        ...
        <div><p class="text-lg font-semibold text-teal-600">{{ formatMoney(totalStats.actualTotal) }}</p><p class="text-xs text-stone-400">实际执行</p></div>
      </div>
      <div class="warm-card flex items-center gap-3 !py-3" :class="totalStats.percent > 100 ? 'border-red-300' : totalStats.percent > 80 ? 'border-amber-300' : ''">
        <div class="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center"><UIcon name="i-lucide-percent" class="w-5 h-5 text-amber-500" /></div>
        <div><p class="text-lg font-semibold" :class="totalStats.percent > 100 ? 'text-red-500' : 'text-stone-700'">{{ totalStats.percent }}%</p><p class="text-xs text-stone-400">执行率</p></div>
      </div>
    </div>

    <!-- 筛选 -->
    <div class="flex flex-wrap items-center gap-3 mb-4">
      <input v-model.number="year" type="number" class="px-3 py-2 text-sm rounded-lg border border-stone-200 w-24" @change="page=1; fetchItems()" />
    ...
    <div v-else-if="items.length === 0" class="text-center py-12 text-stone-400">{{ year }} 年还没有预算，加一条？</div>
      ...
      <div v-for="item in items" :key="item.id" class="warm-card flex items-center gap-4">
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1">
            ...
            <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-stone-100 text-stone-500">{{ item.category }}</span>
          </div>
          <div class="flex items-center gap-4 text-xs text-stone-400">
            <span class="font-medium text-stone-600">预算 {{ formatMoney(item.amount) }}</span>
          ...
          <!-- 进度条 -->
          <div class="mt-1.5 h-1.5 bg-stone-100 rounded-full overflow-hidden w-full max-w-xs">
            <div
              class="h-full rounded-full transition-all"
              :class="item.usagePercent > 100 ? 'bg-red-400' : item.usagePercent > 80 ? 'bg-amber-400' : 'bg-teal-400'"
              :style="{ width: Math.min(item.usagePercent, 100) + '%' }"
            />
          </div>
          <div class="flex justify-between mt-0.5 w-full max-w-xs">
            <span class="text-[10px] text-stone-400">{{ item.usagePercent }}%</span>
            <span v-if="item.usagePercent > 100" class="text-[10px] text-red-500">超支！</span>
            <span v-else-if="item.usagePercent > 80" class="text-[10px] text-amber-600">接近上限</span>
          </div>
        </div>
      ...
      <template #body>
        <form class="space-y-3" @submit.prevent="handleSave">
          <div class="grid grid-cols-2 gap-3">
            <div><label class="block text-sm text-stone-600 mb-1">名称 <span class="text-red-400">*</span></label><input v-model="form.name" type="text" placeholder="市场推广预算" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" /></div>
            <div><label class="block text-sm text-stone-600 mb-1">类型</label><select v-model="form.type" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 bg-white"><option value="income">收入预算</option><option value="expense">支出预算</option></select></div>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div><label class="block text-sm text-stone-600 mb-1">分类 <span class="text-red-400">*</span></label><select v-model="form.category" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 bg-white"><option value="">选择分类</option><option v-for="c in categories" :key="c.id" :value="c.name">{{ c.name }}</option></select></div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div><label class="block text-sm text-stone-600 mb-1">年度</label><input v-model.number="form.year" type="number" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400" /></div>
            <div><label class="block text-sm text-stone-600 mb-1">月份（可选）</label><select v-model="form.month" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 bg-white"><option :value="undefined">全年</option><option v-for="m in 12" :key="m" :value="m">{{ m }} 月</option></select></div>
          </div>
          <div><label class="block text-sm text-stone-600 mb-1">备注</label><textarea v-model="form.remark" rows="2" placeholder="预算说明..." class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 resize-none" /></div>
        </form>
      </template>
    ...
    <UModal v-model:open="showDeleteModal">
      ...
      <template #body><p class="text-sm text-stone-600">确定要删除「{{ deleteTarget?.name }}」吗？</p></template>
      <template #footer><div class="flex justify-end gap-2"><UButton variant="ghost" color="neutral" @click="showDeleteModal = false">取消</UButton><UButton color="error" :loading="deleteLoading" @click="handleDelete">确认删除</UButton></div></template>
    </UModal>
  </div>
</template>

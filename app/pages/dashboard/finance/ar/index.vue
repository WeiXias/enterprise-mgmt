...
<template>
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-lg font-medium text-gray-800">应收账款</h1>
        <p class="text-sm text-gray-400 mt-0.5">合同付款计划 + 销售订单确认金额</p>
      </div>
    ...
    <div class="flex flex-wrap items-center gap-3 mb-4">
      <div class="relative flex-1 min-w-[200px] max-w-xs">
        ...
        <input v-model="keyword" type="text" placeholder="搜客户名、合同名..." class="w-full pl-9 pr-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400 transition-colors" @input="onSearchInput" />
      </div>
    ...
    <div v-else-if="items.length === 0" class="text-center py-12 text-gray-400">
      <UIcon name="i-lucide-receipt" class="w-10 h-10 mx-auto mb-2 text-gray-300" />
      ...
      <div v-for="item in items" :key="item.id" class="warm-card flex items-center gap-4">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-0.5">
            <span class="text-sm font-medium text-gray-800">{{ item.customerName || '未知客户' }}</span>
            <span class="text-xs text-gray-400">{{ item.contractName || '销售订单' }}</span>
          </div>

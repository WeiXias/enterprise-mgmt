...
<template>
  <div>
    <div class="mb-6">
      <h1 class="text-lg font-medium text-content-primary">财务报表</h1>
      <p class="text-sm text-content-muted mt-0.5">看看经营情况怎么样</p>
    </div>
    ...
    <div v-show="activeTab === 0">
      <div class="flex items-center gap-3 mb-4">
        <span class="text-sm text-content-secondary">年份：</span>
        <input v-model.number="year" type="number" class="input-base w-24" @change="fetchMonthly" />
      </div>

      <div v-if="loadingMonthly" class="text-center py-12 text-content-muted">加载中...</div>
      <div v-else>
        <!-- 汇总 -->
        ...
        <!-- 月度趋势柱状图 -->
        <div v-if="monthlyData.length > 0" class="em-card mb-6">
          <h3 class="text-sm font-medium text-content-secondary mb-4">月度趋势</h3>
          <div class="flex items-end gap-2 h-40">
            <div v-for="row in monthlyData" :key="row.month" class="flex-1 flex flex-col items-center gap-1">
              ...
              <span class="text-[10px] text-content-muted">{{ row.month?.slice(5) }}月</span>
            </div>
          </div>
        ...
        <div v-else class="em-card overflow-hidden">
          <table class="w-full text-sm">
            <thead><tr class="border-b border-line-light text-left text-xs text-content-muted"><th class="py-2 px-4">月份</th><th class="py-2 px-4 text-right">收入</th><th class="py-2 px-4 text-right">支出</th><th class="py-2 px-4 text-right">利润</th></tr></thead>
            <tbody>
    ...
    <div v-show="activeTab === 1">
      <div v-if="loadingProject" class="text-center py-12 text-content-muted">加载中...</div>
      <EmptyState v-if="projectData.length === 0" message="还没有项目利润数据，合同转项目后这里就能看到了" />
      <div v-else class="em-card overflow-hidden">
        <table class="w-full text-sm">
          <thead><tr class="border-b border-line-light text-left text-xs text-content-muted"><th class="py-2 px-4">项目</th><th class="py-2 px-4 text-right">收入</th><th class="py-2 px-4 text-right">支出</th><th class="py-2 px-4 text-right">利润</th></tr></thead>
          <tbody>

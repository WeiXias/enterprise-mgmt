...
<template>
  <div>
    <div class="mb-6">
      <h1 class="text-lg font-medium text-stone-800">财务报表</h1>
      <p class="text-sm text-stone-400 mt-0.5">看看经营情况怎么样</p>
    </div>
    ...
    <div v-show="activeTab === 0">
      <div class="flex items-center gap-3 mb-4">
        <span class="text-sm text-stone-600">年份：</span>
        <input v-model.number="year" type="number" class="px-3 py-2 text-sm rounded-lg border border-stone-200 w-24" @change="fetchMonthly" />
      </div>

      <div v-if="loadingMonthly" class="text-center py-12 text-stone-400">加载中...</div>
      <div v-else>
        <!-- 汇总 -->
        <div class="grid grid-cols-3 gap-4 mb-6">
          <div class="warm-card text-center"><p class="text-xs text-stone-400 mb-1">总收入</p><p class="text-lg font-semibold text-teal-600">{{ formatMoney(monthlyTotals.income) }}</p></div>
          <div class="warm-card text-center"><p class="text-xs text-stone-400 mb-1">总支出</p><p class="text-lg font-semibold text-red-500">{{ formatMoney(monthlyTotals.expense) }}</p></div>
          <div class="warm-card text-center"><p class="text-xs text-stone-400 mb-1">净利润</p><p class="text-lg font-semibold" :class="monthlyTotals.profit >= 0 ? 'text-teal-600' : 'text-red-500'">{{ formatMoney(monthlyTotals.profit) }}</p></div>
        </div>

        <!-- 月度趋势柱状图 -->
        <div v-if="monthlyData.length > 0" class="warm-card mb-6">
          <h3 class="text-sm font-medium text-stone-700 mb-4">月度趋势</h3>
          <div class="flex items-end gap-2 h-40">
            <div v-for="row in monthlyData" :key="row.month" class="flex-1 flex flex-col items-center gap-1">
              ...
              <span class="text-[10px] text-stone-400">{{ row.month?.slice(5) }}月</span>
            </div>
          </div>
          <div class="flex items-center gap-4 mt-3 justify-center text-xs text-stone-400">
        ...
        <div v-else class="warm-card overflow-hidden">
          <table class="w-full text-sm">
            <thead><tr class="border-b border-stone-100 text-left text-xs text-stone-400"><th class="py-2 px-4">月份</th><th class="py-2 px-4 text-right">收入</th><th class="py-2 px-4 text-right">支出</th><th class="py-2 px-4 text-right">利润</th></tr></thead>
            <tbody>
              <tr v-for="row in monthlyData" :key="row.month" class="border-b border-stone-50">
                <td class="py-2 px-4 text-stone-700">{{ row.month }}</td>
    ...
    <div v-show="activeTab === 1">
      <div v-if="loadingProject" class="text-center py-12 text-stone-400">加载中...</div>
      <EmptyState v-if="projectData.length === 0" message="还没有项目利润数据，合同转项目后这里就能看到了" />
      <div v-else class="warm-card overflow-hidden">
        <table class="w-full text-sm">
          <thead><tr class="border-b border-stone-100 text-left text-xs text-stone-400"><th class="py-2 px-4">项目</th><th class="py-2 px-4 text-right">收入</th><th class="py-2 px-4 text-right">支出</th><th class="py-2 px-4 text-right">利润</th></tr></thead>
          <tbody>
            <tr v-for="row in projectData" :key="row.projectId" class="border-b border-stone-50">
              <td class="py-2 px-4 text-stone-700">{{ row.projectName }}</td>
              <td class="py-2 px-4 text-right text-teal-600">{{ formatMoney(row.income) }}</td>
              <td class="py-2 px-4 text-right text-red-500">{{ formatMoney(row.expense) }}</td>
              <td class="py-2 px-4 text-right font-medium" :class="row.profit >= 0 ? 'text-teal-600' : 'text-red-500'">{{ formatMoney(row.profit) }}</td>
            </tr>

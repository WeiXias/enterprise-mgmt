...
definePageMeta({ layout: 'dashboard', title: '提成发放', middleware: ['auth'] })
...
<template>
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-lg font-medium text-stone-800">提成发放</h1>
        <p class="text-sm text-stone-400 mt-0.5">创建和管理提成发放单</p>
      </div>
    ...
    <div v-else-if="payouts.length === 0" class="text-center py-12 text-stone-400">还没有发放单，先创建一张？</div>
    <div v-else class="space-y-2">
      <div v-for="p in payouts" :key="p.id" class="warm-card flex items-center gap-4">
        <div :class="['w-1 h-10 rounded-full flex-shrink-0', p.status === 'confirmed' ? 'bg-teal-400' : 'bg-stone-300']" />
        <div class="flex-1">
          <div class="flex items-center gap-2 mb-0.5">
            <span class="text-sm font-medium text-stone-700">{{ p.periodMonth }} 发放单</span>
          ...
          <div class="flex items-center gap-3 text-xs text-stone-400">
            <span class="font-medium text-stone-600">{{ formatMoney(p.totalAmount) }}</span>
    ...
    <UModal v-model:open="showCreateModal">
      ...
      <template #body>
        <div class="space-y-3">
          <div>
            <label class="block text-sm text-stone-600 mb-1">发放月份 <span class="text-red-400">*</span></label>
            <input v-model="periodMonth" type="month" class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" />
          </div>

          <div v-if="approvedItems.length === 0" class="text-xs text-stone-400 bg-stone-50 p-3 rounded-lg">暂无可发放提成（需要审批通过后才会出现在这里）</div>
          <div v-else>
            <div class="flex items-center justify-between mb-2">
              <label class="text-sm text-stone-600">选择提成记录</label>
              <button class="text-xs text-amber-600 hover:underline" @click="selectAll">{{ selectedIds.size === approvedItems.length ? '取消全选' : '全选' }}</button>
            </div>
            <div class="max-h-64 overflow-y-auto space-y-1">
              <label v-for="item in approvedItems" :key="item.id" class="flex items-center gap-2 p-2 rounded-lg hover:bg-stone-50 cursor-pointer text-xs">
                <input type="checkbox" :checked="selectedIds.has(item.id)" class="rounded border-stone-300 text-amber-500 focus:ring-amber-400" @change="toggleSelect(item.id)" />
                <span class="text-stone-700">{{ item.user?.name }}</span>
                <span class="text-stone-400">-</span>
                <span class="text-stone-600">{{ formatMoney(Number(item.adjustAmount) || Number(item.amount)) }}</span>
              </label>
            </div>
            <div class="mt-2 pt-2 border-t border-stone-100 text-sm text-stone-600">

  ...
  <div>
    ...
    <div class="flex items-center gap-3 mb-4">
      <div class="relative flex-1 min-w-[200px] max-w-xs">
        ...
        <input v-model="keyword" type="text" placeholder="搜凭证号..." class="w-full pl-9 pr-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400" @keydown.enter="fetchEntries" />
      </div>
    ...
    <div v-else class="space-y-2">
      <div v-for="entry in items" :key="entry.id" class="warm-card flex items-center gap-4 cursor-pointer" @click="router.push(`/dashboard/accounting/entries/${entry.id}`)">
        <div :class="['w-1 h-10 rounded-full flex-shrink-0', entry.status === 'posted' ? 'bg-teal-400' : 'bg-gray-300']" />
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-0.5">
            <span class="text-sm font-medium text-gray-800">{{ entry.voucherNo }}</span>
    ...
    <UModal v-model:open="showCreateModal">
      <template #header>录入凭证</template>
      <template #body>
        <form class="space-y-4" @submit.prevent="handleCreate">
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-sm text-gray-600 mb-1">凭证号 <span class="text-red-400">*</span></label>
              <input v-model="form.voucherNo" type="text" placeholder="如 JZ-202606-001" class="w-full px-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400" />
            </div>
            <div>
              <label class="block text-sm text-gray-600 mb-1">日期</label>
              <input v-model="form.date" type="date" class="w-full px-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400" />
            </div>
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">摘要 <span class="text-red-400">*</span></label>
            <input v-model="form.description" type="text" class="w-full px-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400" />
          </div>
          <div>
            <label class="block text-sm text-gray-600 mb-1">期间</label>
            <input v-model="form.period" type="month" class="w-full px-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400" />
          </div>

          <div>
            <div class="flex items-center justify-between mb-1">
              <label class="text-sm text-gray-600">分录行</label>
            ...
            <div class="space-y-1">
              <div v-for="(line, idx) in form.lines" :key="idx" class="grid grid-cols-12 gap-1 items-center">
                <div class="col-span-4">
                  <select v-model="line.accountId" class="w-full px-1 py-1 text-xs rounded border border-gray-200 focus:outline-none focus:border-brand-400 bg-white">
                    <option value="">选科目</option>
                    <option v-for="a in accounts" :key="a.id" :value="a.id">{{ a.code }} {{ a.name }}</option>
                  </select>
                </div>
                <div class="col-span-3">
                  <input v-model.number="line.debit" type="number" min="0" step="0.01" placeholder="借方" class="w-full px-1 py-1 text-xs rounded border border-gray-200 focus:outline-none focus:border-brand-400" />
                </div>
                <div class="col-span-3">
                  <input v-model.number="line.credit" type="number" min="0" step="0.01" placeholder="贷方" class="w-full px-1 py-1 text-xs rounded border border-gray-200 focus:outline-none focus:border-brand-400" />
                </div>
            ...
            <div class="flex gap-4 mt-2 text-xs">
              <span class="text-blue-600">借方合计：{{ totalDebit().toFixed(2) }}</span>

    ...
    <div v-else class="space-y-2">
      ...
      >
        ...
        <div v-if="isAdminOrManager()" class="flex-shrink-0" @click.stop>
          <input type="checkbox" class="w-3.5 h-3.5 rounded border-line text-brand-500 focus:ring-brand-400" :checked="selectedIds.has(customer.id)" @change="toggleSelect(customer.id)" />
        </div>
      ...
      <template #default>
        <form class="space-y-4" @submit.prevent="handleCreate">
          ...
          <div class="rounded-xl border border-line-light bg-line-light/40 p-4">
            ...
            <div class="mb-3">
              <label class="block text-sm text-content-secondary mb-1">客户名称 <span class="text-danger-600">*</span></label>
              <input v-model="createForm.name" type="text" placeholder="公司或个人名称" class="w-full input-base focus-ring" />
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div class="mb-4">
                <label class="block text-sm text-content-secondary mb-1">行业</label>
                <EnumSelect v-model="createForm.industry" :options="industryOptions" placeholder="选择行业" />
              </div>
              <div class="mb-4">
                <label class="block text-sm text-content-secondary mb-1">注册地址</label>
                <input v-model="createForm.registeredAddress" type="text" placeholder="工商注册地址" class="w-full input-base focus-ring" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div class="mb-4">
                <label class="block text-sm text-content-secondary mb-1">办公地址</label>
                <input v-model="createForm.officeAddress" type="text" placeholder="实际办公地址" class="w-full input-base focus-ring" />
              </div>
              <div class="mb-4">
                <label class="block text-sm text-content-secondary mb-1">备注 <span class="text-content-muted ml-1 font-normal">选填</span></label>
                <input v-model="createForm.remark" type="text" placeholder="随手记点什么" class="w-full input-base focus-ring" />
              </div>
            </div>
          </div>
          ...
          <div class="rounded-xl border border-line-light bg-line-light/40 p-4">
            <div class="flex items-center gap-1.5 mb-3">
              <span class="w-0.5 h-3.5 rounded-full bg-brand-400" />
              <span class="text-sm font-medium text-brand-700">主要联系人</span>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div class="mb-4">
                <label class="block text-sm text-content-secondary mb-1">姓名</label>
                <input v-model="createForm.contactName" type="text" placeholder="联系人姓名" class="w-full input-base focus-ring" />
              </div>
              <div class="mb-4">
                <label class="block text-sm text-content-secondary mb-1">职位</label>
                <input v-model="createForm.contactPosition" type="text" placeholder="职位" class="w-full input-base focus-ring" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div class="mb-4">
                <label class="block text-sm text-content-secondary mb-1">电话</label>
                <input v-model="createForm.contactPhone" type="text" placeholder="手机号" class="w-full input-base focus-ring" />
              </div>
              <div class="mb-4">
                <label class="block text-sm text-content-secondary mb-1">邮箱</label>
                <input v-model="createForm.contactEmail" type="email" placeholder="邮箱" class="w-full input-base focus-ring" />
              </div>
            </div>
          </div>
        </form>
          ...
          <div class="rounded-xl border border-line-light bg-line-light/40 p-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <div class="mb-4">
                ...
                <EnumSelect v-model="editForm.status" :options="statusOptions" placeholder="选择状态" />
              </div>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div class="mb-4">
            ...
            <div class="mb-0">
              <label class="block text-sm text-content-secondary mb-1">备注 <span class="text-content-muted ml-1 font-normal">选填</span></label>
              <input v-model="editForm.remark" type="text" class="w-full input-base focus-ring" />
            </div>
          </div>
        </form>
      </template>

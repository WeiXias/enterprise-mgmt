...
<template>
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-lg font-medium text-stone-800">消息通知</h1>
    ...
    <div class="flex items-center gap-2 mb-4">
      <button
        :class="[
          ...
          isReadFilter === undefined ? 'bg-amber-100 text-amber-700' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'
        ]"
      ...
      <button
        :class="[
          'px-3 py-1.5 text-xs rounded-full transition-colors',
          isReadFilter === false ? 'bg-amber-100 text-amber-700' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'
        ]"
      ...
      <button
        :class="[
          'px-3 py-1.5 text-xs rounded-full transition-colors',
          isReadFilter === true ? 'bg-amber-100 text-amber-700' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'
        ]"
    ...
    <div v-else-if="items.length === 0" class="warm-card text-center py-10">
      <UIcon name="i-lucide-bell-off" class="w-8 h-8 text-stone-300 mx-auto mb-3" />
      <p class="text-sm text-stone-400">暂时没有通知</p>
    </div>
    <div v-else class="space-y-1.5">
      <div
        ...
        :key="notif.id"
        :class="[
          'warm-card !py-3 !px-4 cursor-pointer hover:bg-stone-50 transition-colors flex gap-3',
          !notif.isRead ? 'border-amber-200' : ''
        ]"
      ...
      >
        <div :class="[
          ...
          <UIcon
            ...
            class="w-4 h-4"
            :class="notif.isRead ? 'text-stone-400' : 'text-amber-600'"
          />
        </div>
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2">
            <span :class="['text-sm', notif.isRead ? 'text-stone-500' : 'text-stone-800 font-medium']">
              {{ notif.title }}
            </span>
            <span v-if="!notif.isRead" class="w-1.5 h-1.5 rounded-full bg-amber-500 flex-shrink-0" />
          </div>
          <p v-if="notif.content" class="text-xs text-stone-400 mt-0.5 line-clamp-1">{{ notif.content }}</p>
        </div>
        <div class="text-[10px] text-stone-400 flex-shrink-0 mt-0.5">

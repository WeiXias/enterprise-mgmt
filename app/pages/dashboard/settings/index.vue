...
async function loadVersion() {
  try {
    const res = await $fetch('/api/system/version') as any
    ...
    <div class="flex gap-8">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div v-show="activeTab === 'upgrade'" class="em-card max-w-2xl">
          ...
          <div class="flex items-center gap-3 mb-5 p-4 rounded-xl bg-surface-hover">
            ...
            <div>
              ...
              <p class="text-lg font-medium text-content-primary">{{ appVersion || '加载中...' }}</p>
            </div>
          </div>
            ...
            <div v-else class="space-y-2">
              <div v-for="p in aiProviders" :key="p.id" class="flex items-center justify-between py-2.5 px-3 rounded-lg border border-line-light">
                <div class="flex items-center gap-3">
                  <div class="w-2 h-2 rounded-full" :class="p.isEnabled ? 'bg-emerald-500' : 'bg-content-muted'"></div>
                  <div>
          ...
          <div class="em-card">
            ...
            <div v-else class="space-y-2">
              <div v-for="e in aiEmployees" :key="e.id" class="flex items-center justify-between py-2.5 px-3 rounded-lg border border-line-light">
                <div class="flex items-center gap-3">
                  <div class="w-2 h-2 rounded-full" :class="e.isActive ? 'bg-emerald-500' : 'bg-content-muted'"></div>
                  <div>
                    ...
                    <p class="text-[11px] text-content-muted mt-0.5">{{ e.providerName || e.providerId }} · {{ e.model }}</p>
                  </div>
                </div>
                <div class="flex items-center gap-1">
        ...
        <!-- AI 供应商弹窗 -->
        <UModal v-model:open="providerFormOpen" :ui="{ content: 'rounded-2xl bg-surface-card shadow-elevated sm:max-w-lg' }">
          <template #header><h3 class="text-sm font-medium text-content-primary">{{ providerEditingId ? '编辑供应商' : '新增供应商' }}</h3></template>
        ...
        <!-- 数字员工弹窗 -->
        <UModal v-model:open="employeeFormOpen" :ui="{ content: 'rounded-2xl bg-surface-card shadow-elevated sm:max-w-lg' }">
          <template #header><h3 class="text-sm font-medium text-content-primary">{{ employeeEditingId ? '编辑数字员工' : '新增数字员工' }}</h3></template>
          <div class="space-y-3 p-1">
        ...
        <!-- 删除确认 -->
        <ConfirmDialog v-model:open="providerDeleteOpen" title="确认删除供应商？" description="删除后无法恢复" danger @confirm="deleteProvider" />
        <ConfirmDialog v-model:open="employeeDeleteOpen" title="确认删除数字员工？" description="删除后无法恢复" danger @confirm="deleteEmployee" />
        <!-- 菜单排序 -->
        <div v-show="activeTab === 'sidebar'" class="em-card">
          <h3 class="text-sm font-medium text-content-primary mb-4">侧边栏模块排序</h3>

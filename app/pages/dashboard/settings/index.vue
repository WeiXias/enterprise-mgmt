...
async function loadMenuItems() {
  try {
    const defaultMenus = [
      ...
      { key: 'opportunities', label: '商机', sort: 7 },
      { key: 'products', label: '产品', sort: 8 },
      { key: 'contract_center', label: '合同中心', sort: 9 },
      { key: 'contracts', label: '合同列表', sort: 10 },
      { key: 'contract_ledger', label: '合同台账', sort: 11 },
      { key: 'contract_templates', label: '模板管理', sort: 12 },
      { key: 'projects', label: '项目', sort: 13 },
      { key: 'finance', label: '财务', sort: 14 },
      { key: 'inventory', label: '进销存', sort: 15 },
      { key: 'purchases', label: '采购', sort: 16 },
      { key: 'suppliers', label: '供应商', sort: 17 },
    ]

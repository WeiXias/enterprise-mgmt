...
<template>
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-lg font-medium text-stone-800">销售漏斗</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
      <div class="warm-card flex items-center gap-3">
        <div class="w-9 h-9 rounded-lg bg-amber-50 flex items-center justify-center">
          <UIcon name="i-lucide-flag" class="w-5 h-5 text-amber-500" />
        </div>
        <div>
          <p class="text-lg font-semibold text-stone-800">{{ funnelData.total }}</p>
          <p class="text-xs text-stone-400">商机总数</p>
        </div>
      </div>
      <div class="warm-card flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-stone-400">总金额</p>
        </div>
      </div>
      <div class="warm-card flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-stone-400">赢单率</p>
        </div>
      </div>
    </div>
    ...
    <div v-else class="warm-card">
      <h3 class="text-sm font-medium text-stone-700 mb-4">各阶段分布</h3>
      <div class="space-y-3">
        <div
          v-for="(stage, i) in funnelData.stages"
          :key="stage.status"
          class="flex items-center gap-4 cursor-pointer hover:bg-stone-50 rounded-lg p-2 -mx-2 transition-colors"
          :class="{ 'opacity-50': stage.status === 'closed_lost' }"
          @click="selectStage(stage.status)"
        >
          <!-- 阶段色条 -->
          <div
            :class="[stageColors[stage.status] || 'bg-stone-300', 'w-2 h-10 rounded-full flex-shrink-0']"
          />

          <!-- 阶段信息 -->
          <div class="w-24 flex-shrink-0">
            <p class="text-sm font-medium text-stone-700">{{ statusLabels[stage.status] }}</p>
            <p class="text-xs text-stone-400">{{ stage.count }} 个 · {{ formatMoney(stage.totalAmount) }}</p>
          </div>

          <!-- 进度条 -->
          <div class="flex-1 h-6 bg-stone-100 rounded-full overflow-hidden">
            <div
              :class="[stageColors[stage.status] || 'bg-stone-300', 'h-full rounded-full transition-all flex items-center justify-end pr-2']"
          ...
          <div class="w-12 text-right">
            <span v-if="i > 0 && funnelData.stages[i - 1].count > 0" class="text-xs text-stone-400">
    ...
    <UModal v-model:open="stageModalOpen">
      <template v-if="selectedStage" #header>
        {{ statusLabels[selectedStage] }}阶段的商机
      </template>
      <template v-if="selectedStage" #body>
        <div v-if="stageLoading" class="text-center py-6 text-stone-400">马上就好...</div>
        <div v-else-if="stageOpportunities.length === 0" class="text-center py-6 text-stone-400">这个阶段还没有商机</div>
        <div v-else class="space-y-2">
          <NuxtLink
            v-for="opp in stageOpportunities"
            :key="opp.id"
            :to="`/dashboard/opportunities/${opp.id}`"
            class="warm-card flex items-center gap-3 !py-3 !px-4 hover:bg-stone-50 transition-colors block"
          >
            <div class="flex-1 min-w-0">
              <p class="text-sm text-stone-800 truncate">{{ opp.name }}</p>
              <p class="text-xs text-stone-400 mt-0.5">
                {{ opp.customer?.name || '-' }}
                <span v-if="opp.owner?.name" class="ml-2">{{ opp.owner.name }}</span>
              </p>
            </div>
            <p class="text-sm font-medium text-stone-700">{{ formatMoney(opp.estimatedAmount) }}</p>
          </NuxtLink>
        </div>
      </template>

...
<template>
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-lg font-medium text-content-primary">分包合同</h1>
        <p class="text-sm text-content-muted mt-0.5">管理分包合作方的合同</p>
      </div>
    ...
    <div class="flex items-center gap-3 mb-4">
      <EnumSelect v-model="statusFilter" dict="ContractStatus" placeholder="全部状态" @change="page = 1; fetchItems()" />
      <span class="text-xs text-content-muted">共 {{ total }} 条</span>
    </div>

    <!-- 列表 -->
    <div v-if="loading" class="text-center py-12 text-content-muted">马上就好...</div>
    <div v-else-if="items.length === 0" class="text-center py-12 text-content-muted">还没有分包合同，新建一个？</div>
    <div v-else class="space-y-2">
      <NuxtLink v-for="item in items" :key="item.id" :to="`/dashboard/contracts/subcontracts/${item.id}`" class="em-card flex items-center gap-4 hover:shadow-sm transition-shadow">
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-0.5">
            <span class="text-sm font-medium text-content-secondary">{{ item.name }}</span>
            <span class="text-xs text-content-muted">{{ item.code }}</span>
    ...
    >
      <form class="space-y-3" @submit.prevent="handleSave">
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-sm text-content-secondary mb-1">名称 <span class="text-red-400">*</span></label><input v-model="form.name" type="text" placeholder="分包合同名称" class="w-full input-base focus-ring" /></div>
          <div><label class="block text-sm text-content-secondary mb-1">编码</label><input v-model="form.code" type="text" placeholder="自动生成" class="w-full input-base focus-ring" /></div>
        </div>
        <div>
          <label class="block text-sm text-content-secondary mb-1">分包方</label>
          <EnumSelect v-model="form.subcontractPartyId" :options="partyOptions.map(p => ({ value: p.id, label: p.name }))" placeholder="选择分包方" />
        </div>
        <div class="grid grid-cols-3 gap-3">
          <div><label class="block text-sm text-content-secondary mb-1">金额</label><input v-model.number="form.totalAmount" type="number" step="0.01" class="w-full input-base focus-ring" /></div>
          <div><label class="block text-sm text-content-secondary mb-1">税率</label><input v-model.number="form.taxRate" type="number" step="0.01" placeholder="0.06 = 6%" class="w-full input-base focus-ring" /></div>
          <div><label class="block text-sm text-content-secondary mb-1">状态</label><EnumSelect v-model="form.status" :options="[{value:'draft',label:'草稿'},{value:'in_progress',label:'进行中'},{value:'completed',label:'已完成'}]" placeholder="选择状态" /></div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div><label class="block text-sm text-content-secondary mb-1">开始日期</label><input v-model="form.startDate" type="date" class="w-full input-base focus-ring" /></div>
          <div><label class="block text-sm text-content-secondary mb-1">结束日期</label><input v-model="form.endDate" type="date" class="w-full input-base focus-ring" /></div>
        </div>

...
const reminders = ref([{ label: '待跟进客户', count: 0, bgClass: 'bg-brand-50', textClass: 'text-brand-700', to: '/dashboard/customers' }, { label: '即将到期合同', count: 0, bgClass: 'bg-brand-50', textClass: 'text-brand-700', to: '/dashboard/contracts' }, { label: '待完成任务', count: 0, bgClass: 'bg-emerald-50', textClass: 'text-emerald-700', to: '/dashboard/todos' }])
...
async function fetchData() {
  ...
  try {
    ...
    if (res?.code === 0) {
      ...
      reminders.value = [
        { label: '待跟进客户', count: res.data.todayReminders.followUps, bgClass: 'bg-brand-50', textClass: 'text-brand-700', to: '/dashboard/customers' },
...
const kpiItems = computed(() => [
  { label: '客户总数', value: kpi.value.customerTotal ?? 0, icon: 'i-lucide-users', colorClass: 'text-brand-500', bgClass: 'bg-brand-50', to: '/dashboard/customers' },
  ...
  { label: '总商机金额', value: formatMoney(kpi.value.oppTotalAmount ?? 0), icon: 'i-lucide-trending-up', colorClass: 'text-brand-600', bgClass: 'bg-brand-50', to: '/dashboard/opportunities' },
...
<template>
  <div v-if="loading" class="text-center py-12">
    <div class="inline-flex items-center gap-2 text-content-muted">
  ...
  <div v-else>
    ...
    <div class="card mb-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <button @click="isEditingShortcuts = !isEditingShortcuts" class="text-xs text-brand-600 hover:text-brand-700 font-medium transition-colors">
      ...
      <div v-if="isEditingShortcuts" class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
        <button v-for="item in allShortcuts" :key="item.key" @click="toggleShortcut(item.key)"
          :class="['flex flex-col items-center gap-1 p-2 rounded-md border transition-all',
            pinnedKeys.includes(item.key) ? 'border-brand-400 bg-brand-50/50 shadow-sm' : 'border-line-light opacity-50 hover:opacity-75 hover:border-line']">
        ...
        <button @click="toggleShortcut('todos')"
          :class="['flex flex-col items-center gap-1 p-2 rounded-md border transition-all',
            pinnedKeys.includes('todos') ? 'border-brand-400 bg-brand-50/50 shadow-sm' : 'border-line-light opacity-50 hover:opacity-75 hover:border-line']">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      ...
      <div class="em-card">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-sm font-medium text-content-secondary">最近添加的客户</h3>
          <NuxtLink to="/dashboard/customers" class="text-xs text-brand-600 hover:text-brand-700 font-medium transition-colors">查看全部 →</NuxtLink>
        </div>
        <div v-if="recentCustomers.length === 0" class="text-xs text-content-muted py-8 text-center">还没有客户，加一个？</div>
        <div v-else class="space-y-1">
          <NuxtLink v-for="c in recentCustomers" :key="c.id" :to="`/dashboard/customers/${c.id}`" class="flex items-center justify-between px-3 py-2.5 rounded-md hover:bg-surface-hover transition-colors">
            <div class="flex items-center gap-3">
              <div class="w-8 h-8 rounded-md bg-brand-50 flex items-center justify-center flex-shrink-0">
                <span class="text-brand-600 text-xs font-medium">{{ c.name?.charAt(0) }}</span>
              </div>
              <div class="min-w-0">
                <p class="text-sm text-content-primary truncate">{{ c.name }}</p>
                <p class="text-xs text-content-muted">{{ c.industry || '未分类' }}</p>
              </div>
            </div>
            <span class="text-xs text-content-muted flex-shrink-0">{{ (c.createdAt || '').slice(0, 10) }}</span>
          </NuxtLink>
        </div>
      </div>
      ...
      <div class="em-card">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-sm font-medium text-content-secondary">最近商机</h3>
          <NuxtLink to="/dashboard/opportunities" class="text-xs text-brand-600 hover:text-brand-700 font-medium transition-colors">查看全部 →</NuxtLink>
        </div>
        <div v-if="recentOpportunities.length === 0" class="text-xs text-content-muted py-8 text-center">还没有商机，创建一个？</div>
        <div v-else class="space-y-1">
          <NuxtLink v-for="o in recentOpportunities" :key="o.id" :to="`/dashboard/opportunities/${o.id}`" class="flex items-center justify-between px-3 py-2.5 rounded-md hover:bg-surface-hover transition-colors">
            <div class="flex items-center gap-3">
              ...
              <div class="min-w-0">
                <p class="text-sm text-content-primary truncate">{{ o.name }}</p>
                <p class="text-xs text-content-muted">{{ getLabel('OpportunityStatus', o.status) || o.status }}</p>
              </div>
            </div>
            <span class="text-sm text-content-secondary font-medium flex-shrink-0">{{ formatMoney(o.amount) }}</span>
          </NuxtLink>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <!-- 销售漏斗 -->
      <div class="em-card">
        <h3 class="text-sm font-medium text-content-secondary mb-4">
          销售漏斗
          <span class="text-xs text-content-muted ml-2">共 {{ funnelData.total }} 个商机</span>
        </h3>
        <div v-if="funnelData.stages.length === 0" class="text-xs text-content-muted py-8 text-center">暂无数据</div>
        <div v-else class="space-y-2.5">
          <div v-for="stage in funnelData.stages" :key="stage.status" class="flex items-center gap-3">
            <span class="text-xs text-content-secondary w-16 flex-shrink-0">{{ getLabel('OpportunityStatus', stage.status) || stage.status }}</span>
            <div class="flex-1 h-4 bg-surface-hover rounded-full overflow-hidden">
              <div class="h-full bg-brand-500 rounded-full transition-all" :style="{ width: formatPercent(Number(stage.count), funnelData.total) }" />
            </div>
            <span class="text-xs text-content-muted w-12 text-right">{{ stage.count }} 个</span>
          </div>
        </div>
      </div>
    </div>

...
export const CUSTOMER_STATUS_CONFIG: Record<CustomerStatus, { label: string; color: string; dotColor: string }> = {
  potential: { label: '潜在客户', color: 'bg-surface-hover text-content-secondary', dotColor: 'bg-surface-muted' },
  ...
  lost: { label: '已流失', color: 'bg-danger-50 text-danger-600', dotColor: 'bg-danger-400' },
}
...
export const PROJECT_STATUS_CONFIG: Record<ProjectStatus, { label: string; color: string; dotColor: string }> = {
  not_started: { label: '未开始', color: 'bg-surface-hover text-content-secondary', dotColor: 'bg-surface-muted' },
  ...
  delayed: { label: '已延期', color: 'bg-danger-50 text-danger-600', dotColor: 'bg-danger-400' },
}
...
export const PRODUCT_STATUS_CONFIG: Record<ProductStatus, { label: string; color: string }> = {
  ...
  off_shelf: { label: '下架', color: 'bg-surface-hover text-content-muted' },
}
...
export const TODO_PRIORITY_CONFIG: Record<TodoPriorityType, { label: string; color: string; dotColor: string }> = {
  urgent_important: { label: '紧急重要', color: 'bg-danger-50 text-danger-600', dotColor: 'bg-danger-400' },
  urgent_not_important: { label: '紧急不重要', color: 'bg-brand-50 text-brand-600', dotColor: 'bg-brand-400' },
  important_not_urgent: { label: '重要不紧急', color: 'bg-brand-50 text-brand-700', dotColor: 'bg-brand-400' },
  not_urgent_not_important: { label: '不紧急不重要', color: 'bg-surface-hover text-content-muted', dotColor: 'bg-surface-muted' },
}

export const TODO_STATUS_CONFIG: Record<TodoStatusType, { label: string; color: string; dotColor: string }> = {
  todo: { label: '待办', color: 'bg-surface-hover text-content-secondary', dotColor: 'bg-surface-muted' },
  in_progress: { label: '进行中', color: 'bg-brand-50 text-brand-600', dotColor: 'bg-brand-400' },
  completed: { label: '已完成', color: 'bg-teal-50 text-teal-700', dotColor: 'bg-teal-400' },
}
...
export const LIST_COLOR_CONFIG: Record<ListColorType, { label: string; bg: string; text: string; border: string; dot: string }> = {
  ...
  coral: { label: '珊瑚', bg: 'bg-brand-50', text: 'text-brand-600', border: 'border-brand-200', dot: 'bg-brand-400' },
  stone: { label: '石色', bg: 'bg-surface-card', text: 'text-content-secondary', border: 'border-line', dot: 'bg-surface-muted' },

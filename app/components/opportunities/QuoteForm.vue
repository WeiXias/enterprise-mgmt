...
<template>
  <form class="space-y-4" @submit.prevent="$emit('submit')">
    <div>
      <label class="block text-sm text-gray-600 mb-1">报价名称</label>
      <input :value="modelValue.name" type="text" placeholder="报价单名称" class="w-full px-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400"
    ...
    <div>
      ...
      <input :value="modelValue.validUntil" type="date" class="w-full px-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400 focus:ring-1 focus:ring-brand-400"
        @input="$emit('update:modelValue', { ...modelValue, validUntil: ($event.target as HTMLInputElement).value })" />
    </div>
    <!-- Products -->
    <div class="border-t border-gray-100 pt-3">
      <div class="flex items-center justify-between mb-2"><span class="text-xs text-gray-500">产品明细</span><UButton icon="i-lucide-plus" variant="ghost" color="neutral" size="xs" @click="addItem">添加</UButton></div>
      <div v-if="!modelValue.items?.length" class="text-xs text-gray-400 py-2">还没有添加产品</div>
      <div v-else class="space-y-2">
        <div v-for="(item, i) in modelValue.items" :key="i" class="flex items-center gap-2 text-xs">
          <select :value="item.productId" class="flex-1 px-2 py-1 rounded border border-gray-200 text-xs bg-white" @change="onProductChange(i, ($event.target as HTMLSelectElement).value)">
            <option value="">选产品</option><option v-for="p in productOptions" :key="p.id" :value="p.id">{{ p.name }} ({{ formatMoney(p.price) }})</option></select>
          <input :value="item.quantity" type="number" min="1" class="w-14 px-1 py-1 text-center rounded border border-gray-200 text-xs" @input="updateItem(i, 'quantity', Number(($event.target as HTMLInputElement).value))" />
          <span class="w-16 text-right text-gray-500">{{ formatMoney(item.quantity * item.unitPrice * (item.discount || 1)) }}</span>
    ...
    <div class="flex items-center justify-between border-t border-gray-100 pt-3">
      ...
      <span class="text-sm font-medium text-gray-800">{{ formatMoney(totalAmount) }}</span>
    </div>
  </form>
</template>

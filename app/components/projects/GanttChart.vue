...
<template>
  <div>
    ...
    <div v-else-if="tasks.length === 0 && milestones.length === 0" class="text-center py-8 text-content-muted">暂无任务</div>
    <div v-else class="overflow-x-auto">
      <div class="min-w-[600px]">
        <!-- 表头 -->
        <div class="flex items-center gap-3 text-xs text-content-muted font-medium mb-2 px-2">
        ...
        <template v-if="milestones.length > 0">
          <div class="mb-3">
            <div class="text-[10px] text-content-muted font-medium mb-1 px-2">里程碑</div>
          ...
          <div class="flex items-center gap-3 px-2">
            ...
            <div class="w-28 flex-shrink-0 min-w-0">
              <div class="flex items-center gap-1">
                <UIcon v-if="task.parentId" name="i-lucide-link" class="w-3 h-3 text-content-muted flex-shrink-0" />
                <p class="text-sm text-content-primary truncate">{{ task.title }}</p>
              </div>
              <p v-if="task.assigneeName || task.assignee" class="text-[10px] text-content-muted">{{ task.assigneeName || task.assignee }}</p>
            </div>
            ...
            <div class="flex-1 h-7 bg-surface-hover rounded-full overflow-hidden relative">
              ...
              <div v-else class="h-full flex items-center px-2 text-[10px] text-content-muted">

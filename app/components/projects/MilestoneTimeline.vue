...
<template>
  <div>
    ...
    <div v-else-if="milestones.length === 0" class="text-center py-4 text-stone-300 text-xs">暂无里程碑</div>
    <div v-else class="relative">
      <!-- 时间线竖线 -->
      <div class="absolute left-[11px] top-2 bottom-2 w-px bg-stone-200" />
      <div v-for="m in milestones" :key="m.id" class="relative pl-7 pb-4 last:pb-0">
        ...
        <div
          :class="[
            ...
            isCompleted(m) ? 'bg-teal-400 border-teal-400' : isOverdue(m) ? 'bg-red-50 border-red-400' : 'bg-white border-stone-300'
          ]"
        >
        ...
        <div
          :class="[
            'flex items-start justify-between gap-2 cursor-pointer rounded-lg p-1.5 -ml-1.5 hover:bg-stone-50 transition-colors',
            isCompleted(m) ? 'opacity-60' : ''
          ]"
          @click="editable && emit('toggle', m.id, !isCompleted(m))"
        >
          <div class="min-w-0">
            <p :class="['text-sm', isCompleted(m) ? 'text-stone-400 line-through' : 'text-stone-700']">{{ m.name }}</p>
            <p class="text-xs mt-0.5" :class="isOverdue(m) && !isCompleted(m) ? 'text-red-400 font-medium' : 'text-stone-400'">

...
const statusLabels: Record<string, string> = {
  draft: '草稿', submitted: '待审批', approved: '已通过', rejected: '已驳回',
}
...
<template>
  <div>
    ...
    <div v-else-if="logs.length === 0" class="text-center py-8 text-stone-300 text-xs">暂无工时记录</div>
    <div v-else class="space-y-3">
      <div v-for="group in groupByDate(logs)" :key="group.date">
        ...
        <div class="space-y-1">
          <div v-for="l in group.items" :key="l.id" class="flex items-center gap-2 p-2 rounded-lg hover:bg-stone-50 transition-colors">
            <div class="flex-shrink-0 w-7 h-7 rounded-full bg-amber-50 flex items-center justify-center">
              <span class="text-amber-700 text-xs">{{ (l.userName || getMemberName(l.userId)).charAt(0) }}</span>
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-1">
                <span class="text-sm text-stone-700 truncate">{{ l.userName || getMemberName(l.userId) }}</span>
                <span class="text-xs text-stone-500">{{ l.hours }}h</span>
              </div>
            ...
            <span :class="['text-[10px] px-1 py-0.5 rounded-full flex-shrink-0', statusColors[l.status] || '']">{{ statusLabels[l.status] || l.status }}</span>

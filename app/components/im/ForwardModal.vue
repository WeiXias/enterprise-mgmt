  ...
  >
    ...
    <div class="space-y-3">
      <div class="relative">
        ...
        <input v-model="searchKeyword" type="text" placeholder="搜索会话..." class="w-full pl-8 input-base focus-ring" />
      </div>
      <div class="max-h-48 overflow-y-auto space-y-1">
        <button
          v-for="conv in filteredConvs" :key="conv.id"
          class="w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-md hover:bg-brand-50 transition-colors"
        ...
        >
          ...
          <span class="text-sm text-content-secondary truncate">{{ conv.type === 'group' ? conv.title : conv.participant?.name }}</span>
        </button>

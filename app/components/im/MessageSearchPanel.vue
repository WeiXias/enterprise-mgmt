  ...
  <div class="flex flex-col h-full">
    <div class="flex items-center gap-2 px-3 py-2 border-b border-gray-100">
      <div class="relative flex-1">
        ...
        <input v-model="keyword" type="text" placeholder="搜索聊天记录..." class="w-full pl-8 pr-3 py-1.5 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400" @input="onSearchInput" />
      </div>
      <button class="text-xs text-gray-400 hover:text-gray-600" @click="emit('close')">取消</button>
    </div>
    <div class="flex-1 overflow-y-auto">
      <div v-if="loading && results.length === 0" class="text-center py-8 text-xs text-gray-400">搜索中...</div>
      <div v-else-if="!keyword.trim()" class="text-center py-8 text-xs text-gray-400">输入关键词搜索</div>
      <div v-else-if="results.length === 0" class="text-center py-8 text-xs text-gray-400">没有找到相关内容</div>
      <div v-else>
        <button
        ...
        >
          <div class="flex items-center gap-2 mb-1">
            ...
            <span class="text-xs text-gray-800 font-medium">{{ r.sender?.name || '已注销' }}</span>
            <span class="text-[10px] text-gray-400">{{ r.createdAt?.slice(0, 16) }}</span>
          </div>

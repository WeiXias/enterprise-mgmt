...
<template>
  <div>
    <div class="flex items-center justify-between mb-3">
      <span class="text-sm text-content-muted">收款记录 ({{ items.length }})</span>
    ...
    <div v-else-if="items.length === 0" class="text-center py-6 text-content-muted text-xs">暂无收款记录</div>
    <div v-else class="space-y-2">
      <div v-for="pay in items" :key="pay.id" class="em-card flex items-center gap-3">
        <div class="w-2 h-2 rounded-full bg-teal-400 flex-shrink-0" />
        <div class="flex-1 flex items-center gap-4 text-sm"><span class="text-content-secondary font-medium">{{ formatMoney(pay.amount) }}</span><span class="text-xs text-content-muted">{{ formatDate(pay.paymentDate) }}</span><span v-if="pay.paymentMethod" class="text-xs text-content-muted">{{ getLabel('PaymentMethod', pay.paymentMethod) || pay.paymentMethod }}</span></div>

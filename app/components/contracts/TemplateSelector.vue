...
const categoryConfig: Record<string, { label: string; color: string }> = {
  sales: { label: getLabel('ContractTemplateCategory', 'sales'), color: 'bg-blue-50 text-blue-600 border-blue-200' },
  ...
  other: { label: getLabel('ContractTemplateCategory', 'other'), color: 'bg-gray-50 text-gray-600 border-gray-200' },
}
...
<template>
  <div v-if="loading" class="text-center py-6 text-gray-400 text-sm">加载模板中...</div>
  <div v-else-if="templates.length === 0" class="text-center py-6 text-gray-400 text-sm">还没有合同模板</div>
  <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
    ...
    >
      ...
      <h4 class="text-sm font-medium text-gray-800 mb-1">{{ t.name }}</h4>

...
const statusColors: Record<string, string> = { draft: 'bg-gray-100 text-gray-600', approved: 'bg-blue-50 text-blue-600', in_progress: 'bg-brand-50 text-brand-700', completed: 'bg-teal-50 text-teal-700', terminated: 'bg-red-50 text-red-600' }
</script>

<template>
  <div>
    <div v-if="loading" class="text-center py-6 text-gray-400">加载中...</div>
    <div v-else-if="items.length === 0" class="text-center py-6 text-gray-400 text-xs">暂无数据</div>
    <div v-else class="space-y-1.5">
      <div v-for="item in items" :key="item.contractNo" class="warm-card flex items-center gap-3 !py-3 !px-4">
        ...
        <div class="flex-[2] min-w-0"><p class="text-xs text-gray-500 truncate">{{ item.contractNo }}</p><p class="text-sm text-gray-800 truncate">{{ item.name }}</p></div>
        <div class="flex-1 text-sm text-gray-700 truncate">{{ item.customer }}</div>
        ...
        <div class="flex-1 text-sm text-right" :class="item.unreceivedAmount > 0 ? 'text-red-500 font-medium' : 'text-gray-400'">{{ formatMoney(item.unreceivedAmount) }}</div>
        <div class="w-20 flex items-center gap-1">
          <div class="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden"><div :class="['h-full rounded-full', item.paymentProgress >= 100 ? 'bg-teal-400' : 'bg-brand-400']" :style="{ width: item.paymentProgress + '%' }" /></div>

...
<template>
  <div class="warm-card">
    <h3 class="text-sm font-medium text-gray-700 mb-3">审批操作</h3>
    <div v-if="status === 'draft'" class="space-y-3">
      ...
      <div class="flex gap-2">
        <input v-model="reason" type="text" placeholder="驳回原因" class="flex-1 px-3 h-9 text-sm rounded-lg border border-gray-200 focus:outline-none focus:border-brand-400" />
    ...
    <p v-else class="text-sm text-gray-400 text-center py-2">当前状态不可审批</p>
  </div>
</template>

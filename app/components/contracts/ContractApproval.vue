...
<template>
  <div class="em-card">
    <h3 class="text-sm font-medium text-content-secondary mb-3">审批操作</h3>
    <div v-if="status === 'draft'" class="space-y-3">
      ...
      <div class="flex gap-2">
        <input v-model="reason" type="text" placeholder="驳回原因" class="flex-1 input-base focus-ring" />
    ...
    <p v-else class="text-sm text-content-muted text-center py-2">当前状态不可审批</p>
  </div>
</template>

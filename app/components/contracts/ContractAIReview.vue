...
<template>
  <div class="space-y-4">
    ...
    <div v-if="!loading && !reviewing && !latestReview" class="warm-card text-center py-6">
      <UIcon name="i-lucide-bot" class="w-10 h-10 text-stone-300 mx-auto mb-3" />
      <p class="text-sm text-stone-500 mb-2">还没有 AI 审核记录</p>
      <p class="text-xs text-stone-400 mb-4">让 AI 数字员工帮你审核合同条款，评估风险</p>
      <UButton
    ...
    <div v-else-if="reviewing" class="warm-card text-center py-8">
      <UIcon name="i-lucide-loader-circle" class="w-8 h-8 text-amber-500 mx-auto mb-3 animate-spin" />
      <p class="text-sm text-stone-500">AI 正在审核中...</p>
      <p class="text-xs text-stone-400 mt-1">这可能需要几十秒</p>
    </div>

    <!-- 加载中 -->
    <div v-else-if="loading" class="warm-card flex justify-center py-8">
      <UIcon name="i-lucide-loader-circle" class="w-6 h-6 text-stone-300 animate-spin" />
    </div>
    ...
    <template v-else-if="latestReview">
      ...
      <div v-if="latestReview.status === 'failed'" class="warm-card text-center py-6">
        ...
        <p class="text-xs text-stone-400 mb-4">{{ latestReview.errorMessage || '未知错误' }}</p>
        <UButton
      ...
      <div v-else-if="latestReview.result" class="space-y-4">
        ...
        <div class="warm-card">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2">
              <UIcon name="i-lucide-bot" class="w-5 h-5 text-amber-500" />
              <span class="text-sm font-medium text-stone-700">AI 审核结果</span>
            </div>
            <UButton
              icon="i-lucide-rotate-cw"
              color="secondary"
          ...
          <div class="flex items-center gap-3 mb-3">
            <span
              :class="riskBadgeColors[latestReview.result.riskLevel] || 'bg-stone-100 text-stone-600'"
          ...
          <!-- 摘要 -->
          <p class="text-sm text-stone-600 leading-relaxed mb-3">{{ latestReview.result.summary }}</p>
        ...
        <div v-if="latestReview.result.highlights" class="grid grid-cols-2 gap-3">
          <div class="warm-card">
            ...
            <ul class="space-y-1">
              <li v-for="(item, i) in latestReview.result.highlights.positive.slice(0, 5)" :key="'p' + i" class="text-xs text-stone-600 pl-3 leading-relaxed">
                · {{ item }}
              </li>
              <li v-if="latestReview.result.highlights.positive.length === 0" class="text-xs text-stone-400">暂无</li>
            </ul>
          </div>
          <div class="warm-card">
            ...
            <ul class="space-y-1">
              <li v-for="(item, i) in latestReview.result.highlights.negative.slice(0, 5)" :key="'n' + i" class="text-xs text-stone-600 pl-3 leading-relaxed">
                · {{ item }}
              </li>
              <li v-if="latestReview.result.highlights.negative.length === 0" class="text-xs text-stone-400">暂无</li>
            </ul>
          </div>
        </div>
        ...
        <div v-if="latestReview.result.riskFlags && latestReview.result.riskFlags.length > 0" class="warm-card">
          <h4 class="text-xs font-medium text-stone-600 mb-3 flex items-center gap-1">
          ...
          <div class="space-y-2">
            <div
              ...
              :key="i"
              class="border border-stone-100 rounded-lg p-3 bg-stone-50/50"
            >
              <div class="flex items-start justify-between mb-1">
                <span class="text-xs font-medium text-stone-700">{{ flag.clause }}</span>
                <span :class="riskBadgeColors[flag.severity] || 'bg-stone-100 text-stone-600'" class="px-2 py-0.5 rounded-full text-xs">
                  {{ riskLabel(flag.severity) }}
                </span>
              </div>
              <p class="text-xs text-stone-500 mb-1">{{ flag.description }}</p>
              <p v-if="flag.suggestion" class="text-xs text-amber-600">
                建议：{{ flag.suggestion }}
              </p>
            </div>
          </div>
        ...
        <div v-if="latestReview.result.suggestions && latestReview.result.suggestions.length > 0" class="warm-card">
          <h4 class="text-xs font-medium text-stone-600 mb-3 flex items-center gap-1">
            <UIcon name="i-lucide-lightbulb" class="w-3.5 h-3.5 text-amber-500" />
            修改建议
          </h4>
          <ol class="space-y-2">
            <li v-for="(s, i) in latestReview.result.suggestions" :key="i" class="text-xs text-stone-600 pl-1 leading-relaxed">
              <span class="text-stone-400 font-medium">{{ i + 1 }}.</span> {{ s }}
            </li>
          </ol>
        </div>
      </div>

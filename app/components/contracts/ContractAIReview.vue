...
<template>
  <div class="space-y-4">
    ...
    <!-- 加载中 -->
    <div v-else-if="loading" class="em-card flex justify-center py-8">
      <UIcon name="i-lucide-loader-circle" class="w-6 h-6 text-content-muted animate-spin" />
    </div>
      ...
      <div v-else-if="latestReview.result" class="space-y-4">
        ...
        <div class="em-card">
          ...
          <div class="flex items-center gap-3 mb-3">
            <span
              :class="riskBadgeColors[latestReview.result.riskLevel] || 'bg-surface-hover text-content-secondary'"
          ...
          <!-- 摘要 -->
          <p class="text-sm text-content-secondary leading-relaxed mb-3">{{ latestReview.result.summary }}</p>
        ...
        <div v-if="latestReview.result.highlights" class="grid grid-cols-2 gap-3">
          <div class="em-card">
            ...
            <ul class="space-y-1">
              <li v-for="(item, i) in latestReview.result.highlights.positive.slice(0, 5)" :key="'p' + i" class="text-xs text-content-secondary pl-3 leading-relaxed">
                · {{ item }}
              </li>
              <li v-if="latestReview.result.highlights.positive.length === 0" class="text-xs text-content-muted">暂无</li>
            </ul>
          </div>
          <div class="em-card">
            ...
            <ul class="space-y-1">
              <li v-for="(item, i) in latestReview.result.highlights.negative.slice(0, 5)" :key="'n' + i" class="text-xs text-content-secondary pl-3 leading-relaxed">
                · {{ item }}
              </li>
              <li v-if="latestReview.result.highlights.negative.length === 0" class="text-xs text-content-muted">暂无</li>
            </ul>
          </div>
        </div>

        <!-- 风险标记 -->
        <div v-if="latestReview.result.riskFlags && latestReview.result.riskFlags.length > 0" class="em-card">
          <h4 class="text-xs font-medium text-content-secondary mb-3 flex items-center gap-1">
          ...
          <div class="space-y-2">
            <div
              ...
              :key="i"
              class="border border-line-light rounded-xl p-3 bg-surface-hover/50"
            >
              <div class="flex items-start justify-between mb-1">
                <span class="text-xs font-medium text-content-secondary">{{ flag.clause }}</span>
                <span :class="riskBadgeColors[flag.severity] || 'bg-surface-hover text-content-secondary'" class="px-2 py-0.5 rounded-full text-xs">
                  {{ riskLabel(flag.severity) }}
                </span>
              </div>
              <p class="text-xs text-content-muted mb-1">{{ flag.description }}</p>
        ...
        <!-- 修改建议 -->
        <div v-if="latestReview.result.suggestions && latestReview.result.suggestions.length > 0" class="em-card">
          <h4 class="text-xs font-medium text-content-secondary mb-3 flex items-center gap-1">
            <UIcon name="i-lucide-lightbulb" class="w-3.5 h-3.5 text-brand-500" />
            修改建议
          </h4>
          <ol class="space-y-2">
            <li v-for="(s, i) in latestReview.result.suggestions" :key="i" class="text-xs text-content-secondary pl-1 leading-relaxed">
              <span class="text-content-muted font-medium">{{ i + 1 }}.</span> {{ s }}
            </li>
          </ol>
        </div>
      </div>

...
<template>
  <UModal :open="open" @update:open="emit('update:open', $event)">
    ...
    <template #body>
      <div class="space-y-3">
        <div>
          <label class="block text-sm text-stone-600 mb-1">转交给 <span class="text-red-400">*</span></label>
          <div class="relative">
            <div class="relative">
              <UIcon name="i-lucide-search" class="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-stone-400 pointer-events-none" />
              <input v-model="userSearchKeyword" type="text" placeholder="搜索用户..." class="w-full pl-8 pr-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400 bg-white" @focus="loadUsers" @input="onUserSearch" />
            </div>
            <div v-if="userOptions.length > 0" class="mt-2 max-h-48 overflow-y-auto border border-stone-200 rounded-lg divide-y divide-stone-100">
              <button v-for="u in userOptions" :key="u.id" :class="['w-full text-left px-3 py-2 text-sm hover:bg-amber-50 transition-colors flex items-center gap-2', toUserId === u.id ? 'bg-amber-50' : '']" @click="toUserId = u.id">
                ...
                <div class="flex-1 min-w-0"><p class="text-stone-700 truncate">{{ u.name }}</p><p class="text-xs text-stone-400">{{ u.username }} · {{ u.role }}</p></div>
                <UIcon v-if="toUserId === u.id" name="i-lucide-check" class="w-4 h-4 text-amber-500 ml-1" />
              </button>
            </div>
          </div>
        </div>
        <div>
          ...
          <input v-model="reason" type="text" placeholder="可选，说明一下转交原因..." class="w-full px-3 py-2 text-sm rounded-lg border border-stone-200 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400" />
        </div>
      </div>
    </template>
    <template #footer>

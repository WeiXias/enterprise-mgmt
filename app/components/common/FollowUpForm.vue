...
<template>
  <form class="space-y-3" @submit.prevent="$emit('submit')">
    <div>
      <label class="block text-sm text-content-secondary mb-1">跟进方式</label>
      <div class="flex flex-wrap gap-1.5">
        <button v-for="t in followUpTypes" :key="t" type="button"
          :class="['px-3 py-1 text-xs rounded-full transition-colors', modelValue.type === t ? 'bg-brand-100 text-brand-700' : 'bg-surface-hover text-content-muted hover:bg-surface-hover']"
    ...
    <div><label class="block text-sm text-content-secondary mb-1">下次跟进</label>

...
function getFileTypeGroup(fileName: string): 'image' | 'pdf' | 'office' | 'other' {
  const ext = fileName.split('.').pop()?.toLowerCase() || ''
  ...
  <div>
    ...
    >
      ...
      <div v-if="props.uploading" class="space-y-2">
        <UIcon name="i-lucide-loader-2" class="w-6 h-6 text-amber-500 mx-auto animate-spin" />
        <p class="text-sm text-stone-500">上传中...</p>
      </div>
      <div v-else>
        <UIcon name="i-lucide-upload-cloud" class="w-8 h-8 text-stone-300 mx-auto mb-2" />
        <p class="text-sm text-stone-500">拖拽文件到此处，或点击选择</p>
        <p class="text-xs text-stone-400 mt-1">支持 {{ accept?.replace(/\./g, '').replace(/,/g, ', ') }} (最大 {{ maxSize }}MB)</p>
      </div>
    </div>

    <!-- 已上传文件列表 -->
    <div v-if="props.files && props.files.length > 0" class="mt-3 space-y-1">
      <p class="text-xs text-stone-400 mb-1">已上传 {{ props.files.length }} 个文件</p>
      <div
        v-for="file in props.files"
        :key="file.id"
        class="flex items-center justify-between px-3 py-2 rounded-lg bg-stone-50 hover:bg-stone-100 transition-colors group"
      >
        <div class="flex items-center gap-2 min-w-0">
          <UIcon :name="getFileIcon(file.fileName)" class="w-4 h-4 text-stone-400 flex-shrink-0" />
          <a
            href="#"
            class="text-sm text-amber-600 hover:text-amber-700 hover:underline truncate transition-colors"
            @click.prevent="openPreview(file)"
          >{{ file.fileName }}</a>
          <span class="text-xs text-stone-400 flex-shrink-0">{{ formatFileSize(file.fileSize) }}</span>
        </div>
        <UButton
    ...
    <div v-else-if="loading" class="mt-3 text-center text-xs text-stone-400 py-4">加载中...</div>

    <!-- 文件预览弹窗（全屏） -->
    <UModal v-model:open="showPreview" :ui="{ content: 'w-screen h-screen !max-w-none !max-h-none rounded-none' }">
      <template #header>
        <div class="flex items-center justify-between w-full">
          <span class="text-sm font-medium truncate text-stone-700">{{ previewFile?.fileName }}</span>
          <UButton
            icon="i-lucide-x"
            variant="solid"
            color="neutral"
      ...
      <template #body>
        ...
        <!-- PDF -->
        <iframe v-else-if="previewFile && getFileTypeGroup(previewFile.fileName) === 'pdf'" :src="previewUrl" class="w-full border-0" style="height: calc(100vh - 180px)" />
        <!-- Office 文档 -->

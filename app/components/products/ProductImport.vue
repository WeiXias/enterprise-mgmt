...
<template>
  <div>
    <div :class="['border-2 border-dashed rounded-xl p-6 text-center transition-colors', uploading ? 'border-brand-400 bg-brand-50' : 'border-line hover:border-brand-300']">
      ...
      <label for="import-file" class="cursor-pointer">
        ...
        <UIcon v-else name="i-lucide-file-spreadsheet" class="w-8 h-8 text-content-muted mx-auto mb-2" />
        <p class="text-sm text-content-muted">{{ uploading ? '导入中...' : '点击上传 Excel 文件' }}</p>
        <p class="text-xs text-content-muted mt-1">支持 .xlsx .xls .csv</p>
      </label>
    </div>
    <div v-if="result" class="mt-3 em-card space-y-1 text-sm">
      <p class="text-content-secondary">导入结果：<span class="text-teal-600">{{ result.success }}</span> 成功，<span v-if="result.failed > 0" class="text-red-500">{{ result.failed }} 失败</span></p>
      <div v-if="result.errors?.length" class="mt-2 text-xs text-red-500 space-y-0.5">
        ...
        <p v-if="result.errors.length > 5" class="text-content-muted">还有 {{ result.errors.length - 5 }} 条错误...</p>
      </div>
    </div>
  </div>
</template>

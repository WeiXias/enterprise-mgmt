...
<template>
  <div class="em-card">
    <h3 class="text-sm font-medium text-content-secondary mb-3">回款趋势（近6月）</h3>
    <div v-if="loading" class="h-32 bg-surface-hover rounded-md animate-pulse" />
    <div v-else-if="items.length === 0" class="text-xs text-content-muted py-4 text-center">暂无数据</div>
    <div v-else class="space-y-3">
      <div v-for="item in items" :key="item.month" class="flex items-center gap-2">
        <span class="text-xs text-content-muted w-14 flex-shrink-0">{{ item.month }}</span>
        ...
        <div class="text-[10px] text-content-muted w-16 text-right">
          <p class="text-brand-600">{{ formatMoney(item.contracted) }}</p>
          <p class="text-teal-600">{{ formatMoney(item.received) }}</p>
        </div>
      </div>
    </div>
    <div class="flex items-center gap-4 mt-2 pt-2 border-t border-line-light">
      <span class="text-[10px] text-content-muted"><span class="inline-block w-2 h-2 bg-brand-400 rounded-full mr-1" />签约</span>
      <span class="text-[10px] text-content-muted"><span class="inline-block w-2 h-2 bg-teal-400 rounded-full mr-1" />回款</span>
    </div>
  </div>
</template>
